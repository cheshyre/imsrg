...
class RepoContentProcessor:
    ...
    def format_file_block(self, relative_path, file_content, date_modified):
    ...
    def should_skip_path(self, path):
        ...
        if path.is_file():
            return any(
                fnmatch.fnmatch(path.name, pattern) for pattern in self.skip_patterns
            )
    ...
    def process_file(self, file_path):
        ...
        try:
            ...
            date_modified = subprocess.run(
                'git log -1 --pretty="%ci" {}'.format(file_path).split(),
                capture_output=True,
                text=True,
            ).stdout.strip()
            ...
            path_block = self.format_file_block(
                relative_path, file_content, date_modified
            )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--skip_dirs", type=str, nargs="+", help="Additional directories to skip"
    )
...
def main():
    ...
    try:
        ...
        if args.skip_patterns:
            processor.skip_patterns.append(args.skip_patterns)

        # Add any additional skip directories from command line
        if args.skip_dirs:
            processor.skip_dirs.append(args.skip_dirs)
