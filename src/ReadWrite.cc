    ...
    for(int nljt2=0; nljt2<=nljt1; ++nljt2)
    {
      ...
      for(int nljt3=0; nljt3<=nljt1; ++nljt3)
      {
        ...
        for(int nljt4=0; nljt4<=(nljt3==nljt1 ? nljt2 : nljt3); ++nljt4)
        {
          ...
          if (e3+e4 > Emax) break;
          if ( (o1.l + o2.l + o3.l + o4.l)%2 != 0) continue;
          if ( (o1.tz2 + o2.tz2 != o3.tz2 + o4.tz2)) continue;
...
void ReadWrite::Write_me3jp( std::string ofilename, Operator& Hbare, int E1max, int E2max, int E3max)
{
  std::ofstream outfile(ofilename);
  ...
  int nljtmax = orbits_remap.size();


  outfile << "(*** nsuite/me3b/v0.0.0 (Dec 22 2010) me3jp-f2 ***)" << std::endl;
  outfile << std::setiosflags(std::ios::fixed);
  // begin giant nested loops
  int icount = 0;
  for(int nljt1=0; nljt1<nljtmax; ++nljt1)
  {
    ...
    for(int nljt2=0; nljt2<=nljt1; ++nljt2)
    {
      ...
      for(int nljt3=0; nljt3<=nljt2; ++nljt3)
      {
        int c =  orbits_remap[nljt3];
        Orbit & oc = modelspace->GetOrbit(c);
        int ec = 2*oc.n + oc.l;
        ...
        // now loop over possible ket orbits
        for(int nnljt1=0; nnljt1<=nljt1; ++nnljt1)
        {
          int d =  orbits_remap[nnljt1];
          Orbit & od = modelspace->GetOrbit(d);
          int ed = 2*od.n + od.l;

          for(int nnljt2=0; nnljt2 <= ((nljt1 == nnljt1) ? nljt2 : nnljt1); ++nnljt2)
          {
            int e =  orbits_remap[nnljt2];
            Orbit & oe = modelspace->GetOrbit(e);
            int ee = 2*oe.n + oe.l;

            int nnlj3Max = (nljt1 == nnljt1 and nljt2 == nnljt2) ? nljt3 : nnljt2;
            for(int nnljt3=0; nnljt3 <= nnlj3Max; ++nnljt3)
            {
              int f =  orbits_remap[nnljt3];
              Orbit & of = modelspace->GetOrbit(f);
              int ef = 2*of.n + of.l;
              if ( (ed+ee+ef) > E3max) break;
              // check parity
              ...
              for(int Jab = JabMin; Jab <= JabMax; Jab++)
              {
               for(int JJab = JJabMin; JJab <= JJabMax; JJab++)
               {
                ...
                for(int twoJC = twoJCMin; twoJC <= twoJCMax; twoJC += 2)
                {
                    double V = Hbare.ThreeBody.GetME_pn(Jab,JJab,twoJC,a,b,c,d,e,f);
                    outfile << std::setprecision(7) << std::setw(12) << V << " "  ;
                   if ((icount++)%10==9)
                     outfile << std::endl;
                }//twoJ
...
void ReadWrite::Write_NaiveVS1B(std::string filename, Operator &op) {
  std::ofstream outfile(filename);
  if (!outfile.good()) {
    std::cerr << "************************************" << std::endl
              << "**    Trouble opening file  !!!   **" << std::endl
  ...
  std::vector<index_t> valence_orbs(op.modelspace->valence.cbegin(), op.modelspace->valence.cend());
  std::sort(valence_orbs.begin(), valence_orbs.end());

  time_t time_now =
      std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
  //  outfile << "    generated by IMSRG code on " << ctime(&time_now)<<
  //  std::endl;
  outfile << "# VS 1B file -- generated on " << ctime(&time_now);
  outfile << "# orbitals: \n";
  outfile << "num_orbits: " << valence_orbs.size() << "\n";
  outfile << "# index jj l m_tt n\n";
  for (const auto& a : valence_orbs) {
    const auto& oa = op.modelspace->GetOrbit(a);
    outfile << a << " " << oa.j2 << " " << oa.l << " ";
    if (oa.tz2 == -1) {
      outfile << "p ";
    } else {
      outfile << "n ";
  ...
  outfile << "# matrix elements: \n";
  outfile << "# a b ME \n";
  for (const auto& a : valence_orbs) {
    for (const auto& b : valence_orbs) {
      outfile << a << " " << b << " " << op.OneBody(a, b) << "\n";
    }
  }
}


void ReadWrite::Write_NaiveVS2B(std::string filename, Operator &op) {
  std::ofstream outfile(filename);
  if (!outfile.good()) {
    std::cerr << "************************************" << std::endl
              << "**    Trouble opening file  !!!   **" << std::endl
  ...
  //  outfile << "    generated by IMSRG code on " << ctime(&time_now)<<
  //  std::endl;
  outfile << "# VS 2B file -- generated on " << ctime(&time_now);
  outfile << "# orbitals: \n";
  outfile << "num_orbits: " << valence_orbs.size() << "\n";
  outfile << "# index jj l m_tt n\n";
  for (const auto& a : valence_orbs) {
  ...
  outfile << "# matrix elements: \n";
  outfile << "# a b c d JJ ME \n";
  for (const auto& a : valence_orbs) {
    const auto& oa = op.modelspace->GetOrbit(a);
    for (const auto& b : valence_orbs) {
      const auto& ob = op.modelspace->GetOrbit(b);
      for (const auto& c : valence_orbs) {
        const auto& oc = op.modelspace->GetOrbit(c);
        for (const auto& d : valence_orbs) {
          const auto& od = op.modelspace->GetOrbit(d);

          const int jj_min = std::max(std::abs(oa.j2 - ob.j2), std::abs(oc.j2 - od.j2));
          const int jj_max = std::min(std::abs(oa.j2 + ob.j2), std::abs(oc.j2 + od.j2));

          for (int jj = jj_min; jj <= jj_max; jj += 2) {
            outfile << a << " " << b << " " << c << " " << d << " " << jj << " " << op.TwoBody.GetTBME_J(jj / 2, a, b, c, d) << "\n";
          }
        }
      }
    }
...
void ReadWrite::Write_NaiveVS3B(std::string filename, Operator &op) {
  std::ofstream outfile(filename);
  if (!outfile.good()) {
    std::cerr << "************************************" << std::endl
              << "**    Trouble opening file  !!!   **" << std::endl
  ...
  if (op.particle_rank < 3)
  {
    std::cerr << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! << " << std::endl;
    std::cerr << " Oops. Looks like we're trying to write 3body matrix elements from a " << op.particle_rank << "-body operator. For shame..." << std::endl;
    std::cerr << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! << " << std::endl;
    goodstate = false;
    return;
  }
  ...
  //  outfile << "    generated by IMSRG code on " << ctime(&time_now)<<
  //  std::endl;
  outfile << "# VS 3B file -- generated on " << ctime(&time_now);
  outfile << "# orbitals: \n";
  outfile << "num_orbits: " << valence_orbs.size() << "\n";
  outfile << "# index jj l m_tt n\n";
  for (const auto& a : valence_orbs) {
  ...
  outfile << "# matrix elements: \n";
  outfile << "# a b c d e f JJ_ab JJ_de JJ ME \n";
  for (const auto& a : valence_orbs) {
    const auto& oa = op.modelspace->GetOrbit(a);
    for (const auto& b : valence_orbs) {
      const auto& ob = op.modelspace->GetOrbit(b);
      const int jj_ab_min = std::abs(oa.j2 - ob.j2);
      const int jj_ab_max = std::abs(oa.j2 + ob.j2);
      for (const auto& c : valence_orbs) {
        const auto& oc = op.modelspace->GetOrbit(c);
        for (const auto& d : valence_orbs) {
          const auto& od = op.modelspace->GetOrbit(d);
          for (const auto& e : valence_orbs) {
            const auto& oe = op.modelspace->GetOrbit(e);
            const int jj_de_min = std::abs(od.j2 - oe.j2);
            const int jj_de_max = std::abs(od.j2 + oe.j2);
            for (const auto& f : valence_orbs) {
              const auto& of = op.modelspace->GetOrbit(f);

              for (int jj_ab = jj_ab_min; jj_ab <= jj_ab_max; jj_ab += 2) {
                for (int jj_de = jj_de_min; jj_de <= jj_de_max; jj_de += 2) {

                  const int jj_min = std::max(std::abs(jj_ab - oc.j2), std::abs(jj_de - of.j2));
                  const int jj_max = std::min(std::abs(jj_ab + oc.j2), std::abs(jj_de + of.j2));

                  for (int jj = jj_min; jj <= jj_max; jj += 2) {
                    outfile << a << " " << b << " " << c << " " << d << " " << e << " " << f << " " << jj_ab << " " << jj_de << " " << jj << " " << op.ThreeBody.GetME_pn(jj_ab / 2, jj_de / 2, jj, a, b, c, d, e, f) << "\n";
                  }
                }
              }
            }
     ...
     for (auto& ibra: tbc.GetKetIndex_vv() )
     {
       ...
       for (auto& iket: tbc.GetKetIndex_vv() )
       {
         ...
         intfile << std::setw(wint) << a_ind << std::setw(wint) << b_ind
           ...
           << std::setiosflags(std::ios::scientific) << std::setprecision(pdouble) << tbme
           << std::endl;
       }
     }
   }
